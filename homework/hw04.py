...
def balanced(m):
    ...
    if is_weight(m):
        ...
        end_left_side, end_right_side = end(left_side), end(right_side)
        torque_left, torque_right = length(left_side) * total_weight(end_left_side), length(right_side) * total_weight(end_right_side)
        return balanced(end_left_side) and balanced(end_right_side) and torque_left == torque_right
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        left_side, right_side = left(m), right(m)
        end_left_side, end_right_side = end(left_side), end(right_side)
        left_tree, right_tree = totals_tree(end_left_side), totals_tree(end_right_side)
