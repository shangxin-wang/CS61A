    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                resided_ant = self.ant
                if resided_ant.is_container == True and insect.is_container == False:
                    if resided_ant.can_contain(insect):
                        resided_ant.contain_ant(insect)
                    else:
                        assert self.ant is None, 'Two ants in {0}'.format(self)
                elif resided_ant.is_container == False and insect.is_container == True:
                    if insect.can_contain(resided_ant):
                        insect.contain_ant(resided_ant)
                        self.ant = insect
                    else:
                        assert self.ant is None, 'Two ants in {0}'.format(self)
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        counter = 0
        while counter < self.min_range and checkplace != hive:
            checkplace = checkplace.entrance
            counter += 1
        while self.min_range <= counter <= self.max_range and checkplace != hive:
            if checkplace.bees:
                return random_or_none(checkplace.bees)
            else:
                checkplace = checkplace.entrance
                counter += 1
        return None
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if self.ant is not None and self.ant.is_watersafe == False:
            self.ant.reduce_armor(self.ant.armor)

        if self.bees is not None:
            for b in self.bees:
                if b.is_watersafe == False:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_true:
            ...
            while behind_queen is not None:
                if behind_queen.ant:
                    protected_ant = behind_queen.ant
                    if protected_ant.is_container is False:
                        if protected_ant.already_doubled == False:
                            protected_ant.damage *= 2
                            protected_ant.already_doubled = True
                    if protected_ant.is_container is True:
                        hidden_ant = protected_ant.contained_ant
                        if protected_ant.already_doubled == False:
                            protected_ant.damage *= 2
                            protected_ant.already_doubled = True
                        if hidden_ant is not None and hidden_ant.already_doubled == False:
                            hidden_ant.damage *= 2
                            hidden_ant.already_doubled = True
