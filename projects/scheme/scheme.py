...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        binding_eval = scheme_eval(expressions.second.first,env)
        env.define(target, binding_eval)
        return target
        # END PROBLEM 6
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        ...
        first_expression = scheme_eval(expressions.first, env)
        if scheme_truep(first_expression):
            return first_expression
        elif scheme_falsep(first_expression):
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        value = scheme_eval(bindings.first.second.first, env)
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, args, env):
        mu_frame = env.make_child_frame(self.formals, args)
        mu_frame.parent = env
        return mu_frame
    # END PROBLEM 16
