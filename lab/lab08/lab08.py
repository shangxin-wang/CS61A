...
def make_to_string(front, mid, back, empty_repr):
    ...
    def converter(lst):
        if lst is Link.empty:
            ...
            return front + str(lst.first) + mid + converter(lst.rest) + back
    return converter
...
def tree_map(fn, t):
    ...
    if t.is_leaf():
        ...
        map_b = [tree_map(fn,  b) for b in t.branches]
        return Tree(fn(t.label), map_b)
...
def long_paths(tree, n):
    ...
    if n <=0 and tree.is_leaf():
        ...
        for b in tree.branches:
            path_lst +=  long_paths(b, n-1)
        return [Link(tree.label, p) for p in path_lst]
